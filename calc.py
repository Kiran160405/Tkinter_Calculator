...
for b in buttons:
    ...
    tk.Button(root,text=b,command=cmd,font=("Arial", 14),height=2,width=5,bg="black" if b in "+-/*=" else "gray",fg="white", bd=0).grid(row=r, column=c, pady=6,padx=6)
    c += 1
    if c == 4:
        r += 1
        c = 0
...
tk.Button(root, text="Backspace",font=("Arial",14),
          command=backspace, bg="black",fg="white",
